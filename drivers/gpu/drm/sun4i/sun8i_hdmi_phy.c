...
static int sun8i_hdmi_phy_probe(struct platform_device *pdev)
{
	...
	{ /* just a hack to work hdmi and tvd together */
		struct regulator *vcc = devm_regulator_get(dev, "tvd");
		if (!IS_ERR(vcc)) {
			dev_info(dev, "Enable tvd vcc\n");
			if (regulator_enable(vcc))
				dev_info(dev, "Could not enable tvd vcc\n");
		} else {
			dev_info(dev, "Could not get tvd vcc\n");
		}
	}
