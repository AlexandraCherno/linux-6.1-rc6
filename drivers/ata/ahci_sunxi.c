...
static int ahci_sunxi_hotplug = 0;
module_param_named(hotplug, ahci_sunxi_hotplug, int, 0644);
MODULE_PARM_DESC(hotplug,
	"AHCI SUNXI hot-plug support (0=Don't support, 1=support)");
...
static void ahci_sunxi_error_handler(struct ata_port *ap)
{
	...
	if (ahci_sunxi_hotplug || hpriv->plat_data)
		return;

	/* Avoid repeat ^ */
	hpriv->plat_data = hpriv;
...
static struct ata_port_operations ahci_sunxi_ops = {
	.inherits	= &ahci_platform_ops,
	.error_handler	= ahci_sunxi_error_handler,
};
