...
static int cedrus_open(struct file *file)
{
	...
	ctx->fh.m2m_ctx = v4l2_m2m_ctx_init(ctx->is_enc ?
		dev->m2m_dev_enc : dev->m2m_dev_dec, ctx, &cedrus_queue_init);
...
static struct v4l2_m2m_dev *cedrus_m2m_register(struct cedrus_dev *dev,
				struct video_device *vfd, char *name, int function)
{
	...
	snprintf(vfd->name, sizeof(vfd->name), "%s", name);
	if (function == MEDIA_ENT_F_PROC_VIDEO_ENCODER) {
	...
	if (ret) {
		v4l2_err(&dev->v4l2_dev,
			"Failed to register %s video device\n", name);
		return NULL;
	}
	v4l2_info(&dev->v4l2_dev,
		  "Device %s registered as /dev/video%d\n", name, vfd->num);

	m2m_dev = v4l2_m2m_init(&cedrus_m2m_ops);
	if (IS_ERR(m2m_dev)) {
		v4l2_err(&dev->v4l2_dev,
			 "Failed to initialize %s m2m_dev\n", name);
...
static int cedrus_probe(struct platform_device *pdev)
{
	...
	dev->vfd_dec = cedrus_video_device;
	dev->vfd_enc = cedrus_video_device;
	dev->dev = &pdev->dev;
	dev->pdev = pdev;
	...
	dev->m2m_dev_dec = cedrus_m2m_register(dev, &dev->vfd_dec,
			"cedrus-dec", MEDIA_ENT_F_PROC_VIDEO_DECODER);
	if (!dev->m2m_dev_dec) {
		ret = -ENODEV;
		goto err_mdev_unreg;
	}

	dev->m2m_dev_enc = cedrus_m2m_register(dev, &dev->vfd_enc,
			"cedrus-enc", MEDIA_ENT_F_PROC_VIDEO_ENCODER);
	if (!dev->m2m_dev_enc) {
		ret = -ENODEV;
		goto err_m2m_dec;
	}
