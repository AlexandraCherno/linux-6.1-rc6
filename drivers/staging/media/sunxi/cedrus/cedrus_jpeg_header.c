 ...
 */
static const unsigned char cedrus_jpeg_header[] = {
	/* SOI */
	0xff, 0xd8,
 ...
 * "sizeof(cedrus_jpeg_header)". The two must be equal.
 */
static_assert(sizeof(cedrus_jpeg_header) == JPEG_HEADER_SIZE);

/*
 * cedrus_jpeg_header is padded with a COM segment, so that the payload
 ...
 * to directly.
 */
static_assert(IS_ALIGNED(sizeof(cedrus_jpeg_header), 8),
		       ...
		       const unsigned char *tab, int scale)
{
	...
	for (i = 0; i < JPEG_QUANT_SIZE; i++) {
		...
		reordered_q_tab[i] = jpeg_scale_qp(tab[i], scale);
	}
}

static void jpeg_set_quality(struct cedrus_jpeg_ctx *ctx)
{
	int scale;
...
void cedrus_jpeg_header_assemble(struct cedrus_jpeg_ctx *ctx)
{
	...
	memcpy(buf, cedrus_jpeg_header, sizeof(cedrus_jpeg_header));
